...
def _build_translate_prompt(translate: str, data: dict) -> str:
    return f"""
                ...
                - 包含的纯数字，单字母，纯型号编号、纯单位符号（如 16.2V、3680mAh、>85%、/、1、A），不需要翻译，只需要翻译其他部分。
                - 只有纯数字、单个字母、纯型号编号、纯单位符号（如 16.2V、3680mAh、>85%、/、1、A）才允许 value 等于 key 原样保留。
                - value 不得为空字符串。
                - key 必须原样保留，不得改写、不得纠正错别字、不得增删任何字符（否则会无法对回原文）。
                翻译示例：
                `translate = "中文翻译成英文"`， 示例：
                `translate.json`
                ```json
                {{
                    "1": "1",
